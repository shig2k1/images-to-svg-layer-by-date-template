...
const SVG_TEMPLATE = `<?xml version="1.0" encoding="utf-8"?>
  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1000 883" style="enable-background:new 0 0 1000 883;" xml:space="preserve">
    ...
    <![CDATA[
      ...
      // if you want to test a particular layer, change the date below to a value between 1 - 31 & remove the // prefix
      //date = 30
...
const BG = `<image style="overflow:visible;enable-background:new    ;" width="1000" height="1000" id="/*ID*/" href="data:image/png;base64,/*HREF*/" />`
...
async function readDirAndConvert () {
  ...
  fs.readdir(
    ...
    async (err, files) => {
      ...
      const reorganisedFiles = files.sort((a, b) => transformFileName(a) > transformFileName(b) ? 1 : -1)
      ...
      for (let file of reorganisedFiles) {
        ...
        imgSvg += IMG.replace('/*ID*/', file.split('.')[0]).replace('/*HREF*/', base64)
      }
